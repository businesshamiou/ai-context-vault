...
Une mesure du 2026-08-20 sur les 18 fichiers Mission actifs du registre (`missions/MISSION-INDEX.md` de `workshop-build`) a établi que ce vide a produit un usage divergent : `status` porte tantôt une valeur d'autorisation (`AUTHORIZED`), tantôt une valeur d'exécution (`COMPLETED`), selon le fichier.
...
**D2 — Le registre fait foi.** `missions/MISSION-INDEX.md` est la seule source de l'état d'exécution d'une Mission. Aucun autre emplacement ne fait autorité sur ce point.
