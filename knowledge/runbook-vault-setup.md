...
- **2026-08-24, Mission 040** — Éradication de Graphify, exécution des gestes de retrait planifiés par la [Décision du 2026-08-23](../decisions/DECISION-2026-08-23-184200-graphify-graph-role-withdrawal.md) : hooks Git et merge driver retirés (`graphify hook uninstall`), `graphify-out/` et `.graphifyignore` (deux dépôts) supprimés, paquet `graphifyy` désinstallé (`uv tool uninstall`), `GEMINI_API_KEY` retiré de `.env.example` (aucun autre consommateur trouvé), sections `## graphify` retirées d'`AGENTS.md`/`CLAUDE.md` du Vault, §4 requalifié ci-dessus. Voir le [rapport 040](../../workshop-build/workshop-production/reports/).
